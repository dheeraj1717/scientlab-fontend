...
export const getDeviceStats = async (device, location, token) => {
    return new Promise(async (resolve, reject) => {
        try {
            const response = await fetch(`${BASE_URL}/stats?device=${device}&location=${location}`, {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
export const getChartMetrics = async (request, token, type) => {
  return new Promise(async (resolve, reject) => {
      try {
          ...
          const response = await fetch(`${BASE_URL}/${url}`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify(request) });
